...
func (t *bufferedObjectStream) ReadObject(v interface{}) error {
	fmt.Printf("bufferedObjectStream.ReadObject %v\n", v)
...
func (VSCodeObjectCodec) WriteObject(stream io.Writer, obj interface{}) error {
	fmt.Println("VSCodeObjectCodec WriteObject")
	data, err := json.Marshal(obj)
	if err != nil {
		return err
	}
...
func (VSCodeObjectCodec) ReadObject(stream *bufio.Reader, v interface{}) error {
	...
	for {
		fmt.Printf("VSCodeObjectCodec for loop start\n")
		line, err := stream.ReadString('\r')
		if err != nil {
			fmt.Printf("VSCodeObjectCodec error: %v\n", err)
			return err
		}
		fmt.Printf("VSCodeObjectCodec line: %v\n", line)
		b, err := stream.ReadByte()
		fmt.Printf("VSCodeObjectCodec readbyte: %v\n", b)
		if err != nil {
			return err
		}
		if b != '\n' {
